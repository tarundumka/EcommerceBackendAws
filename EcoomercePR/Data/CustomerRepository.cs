using EcommercePR.DTOs;
using EcommercePR.Models;
...
namespace EcommercePR.Data
{
    public class CustomerRepository
    {
        ...
        public async Task<Customer> GetCustomerById(int cu_id)
        {
            var query = "Select Customerid,name,email,address from customers were customerid=@cu_id and isdeleted=0";
            Customer? customer = null;
            using (var connection = _connectionFactory.CreateConnection())
            {
                await connection.OpenAsync();
                using (var command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@CustomerId", cu_id);
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            customer = new Customer
                            {
                                ...
                                Email = reader.GetString(reader.GetOrdinal("Address")),
                                Address = reader.GetString(reader.GetOrdinal("Address")),
                                IsDeleted = false
